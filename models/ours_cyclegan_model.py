...
class OursCycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'inter_iter']
    ...
    def forward(self, iter=-1):
        ...
        if self.isTrain:
            if iter == 0:
                ...
                '''G*F*F*G'''
                self.fake_B.append(self.netG_A(torch.cat([self.real_A, self.real_A], 1)))
                # print('fake b shape {}'.format(self.fake_B[-1].shape))
                self.rec_A.append(self.netG_B(torch.cat([self.fake_B[-1], self.fake_B[-1]], 1)))
                self.fake_A.append(self.netG_B(torch.cat([self.real_B, self.real_B], 1)))  # G_B(B)
                self.rec_B.append(self.netG_A(torch.cat([self.fake_A[-1], self.fake_A[-1]], 1)))  # G_A(G_B(B))
                # '''F*F*G*G'''
                # self.fake_B.append(self.netG_A(torch.cat([self.real_A, self.real_A], 1)))
                # # print('fake b shape {}'.format(self.fake_B[-1].shape))
                # self.rec_A.append(self.netG_B(torch.cat([self.fake_B[-1], self.fake_B[-1]], 1)))
                ...
                # in_g_a = torch.cat([self.real_A, self.fake_B[-1]], 1)
                # in_g_b = torch.cat([self.real_B, self.fake_A[-1]], 1)
                '''G*F*F*G'''
                self.fake_B.append(self.netG_A(torch.cat([self.real_A, self.fake_B[-1]], 1)))
                self.rec_A.append(self.netG_B(torch.cat([self.fake_B[-1], self.rec_A[-1]], 1)))  # G_B(G_A(A))
                self.fake_A.append(self.netG_B(torch.cat([self.real_B, self.fake_A[-1]], 1)))  # G_B(B)
                self.rec_B.append(self.netG_A(torch.cat([self.fake_A[-1], self.rec_B[-1]], 1)))  # G_A(G_B(B))
                '''F*F*G*G'''
                # self.fake_B.append(self.netG_A(torch.cat([self.real_A, self.fake_B[-1]], 1)))
                # self.rec_B.append(self.netG_A(torch.cat([self.fake_A[-1], self.rec_B[-1]], 1)))  # G_A(G_B(B))
                # self.rec_A.append(self.netG_B(torch.cat([self.fake_B[-1], self.rec_A[-1]], 1)))  # G_B(G_A(A))
            ...
            if iter == 0:
                self.fake_B = self.netG_A(torch.cat([self.real_A, self.real_A], 1))  # G_A(A)
                self.rec_A = self.netG_B(torch.cat([self.fake_B, self.fake_B], 1))  # G_B(G_A(A))
                self.fake_A = self.netG_B(torch.cat([self.real_B, self.real_B], 1))  # G_B(B)
                self.rec_B = self.netG_A(torch.cat([self.fake_A, self.fake_A], 1))  # G_A(G_B(B))
            else:
                self.fake_B = self.netG_A(torch.cat([self.real_A, self.fake_B], 1))  # G_A(A)
                self.rec_A = self.netG_B(torch.cat([self.fake_B, self.rec_A], 1))  # G_B(G_A(A))
                self.fake_A = self.netG_B(torch.cat([self.real_B, self.fake_A], 1))  # G_B(B)
                self.rec_B = self.netG_A(torch.cat([self.fake_A, self.rec_B], 1))  # G_A(G_B(B))
        # self.fake_B[-1] =
        # self.fake_A[-1] =
        # self.rec_A[-1] =
        # self.rec_B[-1] =
    ...
    def backward_G(self, iter):
        ...
        self.loss_inter_iter = 0
        if iter >= 1:
            '''new inter-iter loss'''
            self.loss_inter_iter = self.inter_iter_loss(self.fake_B[-1], self.fake_B[-2]) + \
        ...
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_inter_iter
